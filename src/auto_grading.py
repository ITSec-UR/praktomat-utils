...
def grade_solutions(conn, task_id, admin_id, id_passed=None, id_failed=None):
    ...
    plagiarism_comment = (
        public_comment + "\n\nNote: This submission was marked as plagiarism and has been automatically failed."
    )

    query_grade_passed = f"""
    INSERT INTO attestation_attestation (
        created, public_comment, private_comment, final, published, published_on, author_id, final_grade_id, solution_id
    )
    ...
    WHERE accounts_user.user_ptr_id = solutions_solution.author_id
      ...
      AND solutions_solution.plagiarism = 'f'
      AND COALESCE((
            SELECT bool_and(passed)
            FROM checker_checkerresult
            WHERE solution_id = solutions_solution.id
              AND object_id NOT IN (SELECT id FROM checker_checkstylechecker)
          ), true)
      AND tasks_task.id = {task_id}
      AND NOT EXISTS (
            SELECT 1 FROM attestation_attestation
            WHERE solution_id = solutions_solution.id
          )
    RETURNING id;
    """
    ...
    SELECT now(), '{public_comment}', '', 't', 't', now(), {admin_id}, {id_failed}, solutions_solution.id
    FROM accounts_user, solutions_solution, tasks_task
    WHERE accounts_user.user_ptr_id = solutions_solution.author_id
      AND tasks_task.id = solutions_solution.task_id
      AND solutions_solution.final = 't'
    ...
    query_grade_failed_plagiarism = f"""
    INSERT INTO attestation_attestation (
        created, public_comment, private_comment, final, published, published_on, author_id, final_grade_id, solution_id
    )
    SELECT now(), '{plagiarism_comment}', '', 't', 't', now(), {admin_id}, {id_failed}, solutions_solution.id
    FROM solutions_solution
    WHERE solutions_solution.final = 't'
      AND solutions_solution.plagiarism = 't'
      AND solutions_solution.task_id = {task_id}
      AND NOT EXISTS (
            SELECT 1 FROM attestation_attestation
            WHERE solution_id = solutions_solution.id
          )
    ...
    attestated_ids.extend(run_sql(conn, query_grade_failed_plagiarism))
