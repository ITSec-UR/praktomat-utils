...
def grade_solutions(conn, task_id, admin_id, id_passed, id_failed):
...
def get_solutions(conn, task_id):
...
def get_attestation(conn, solution_id):
...
def write_annotated_solution_files(conn, task_id, work_data):
    """Writes annotated solution files to the database."""
    for solution_id in get_solutions(conn, task_id):
        attestation_id = get_attestation(conn, solution_id)
        ...
        for solution_file_id, file_path in solution_files:
            ...
            try:
                with open(os.path.join(work_data, file_path), 'r', encoding='utf-8') as f:
                    content = f.read()
                run_sql(
                    conn, "INSERT INTO attestation_annotatedsolutionfile (content, attestation_id, solution_file_id) VALUES (%s, %s, %s);", (content, attestation_id, solution_file_id))
...
def get_tasks(conn, task_name=None, rating_scale=None, wait_days=0, interval_days=30):
    ...
    interval_days_filter = f"AND publication_date > now() - INTERVAL '{wait_days + interval_days} DAY'" if interval_days else ""
...
def run():
    ...
    for rating_name in rating_names:
        ...
        for task_regex in task_regexes:
            ...
            if tasks is None:
                raise ValueError(
                    f"Couldn't find any tasks with task regex = {task_regex} and rating_name = {rating_name}")
            for task in tasks:
                task_id = task[0]
                grade_solutions(conn, task_id, admin_id, id_passed, id_failed)
                write_annotated_solution_files(conn, task_id, work_data)
    conn.close()
